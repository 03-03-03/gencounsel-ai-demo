...
const CASE_LIBRARY = {
  case1: {
    ...
    previousTests: [
      '患者本人此前未做过明确的遗传学检测。',
      '患者本人未系统评估过凝血因子Ⅷ水平。',
      '患者已携带舅舅既往病历和凝血因子资料复印件，但其中没有可直接复核的F8家系致病变异报告。'
    ],
...
case6: {
  ...
  previousTests: [
    ...
    '患者已携带父亲外院诊断资料和HTT检测报告摘要，但本人尚未接受预测性检测。'
  ],
...
case8: {
  ...
  previousTests: [
    ...
    '患者本人尚未完全弄清楚具体遗传方式。',
    '近期眼科专科检查资料已随身带来；目前没有母系亲属的遗传检测报告。'
  ],
...
case9: {
  ...
  previousTests: [
    '已进行精液检查，结果异常，正式报告已随身带来。',
    '尚未完成系统性激素水平评估。',
    '尚未进行外周血染色体核型分析。',
    '外院医生建议进一步评估内分泌和染色体原因。'
  ],
...
function buildPatientPrompt(caseData) {
  const materialAvailability = {
    case1: '你已随身带来舅舅的既往病历和凝血因子资料。医生询问时必须回答“带了，可以查看”；资料中没有明确F8家系变异报告。',
    case2: '你已随身带来夫妻双方婚前体检血常规和红细胞指数报告。医生询问时必须回答“带了，可以查看”。',
    case3: '你已随身带来既往流产就诊和组织处理资料。医生询问时必须回答“带了，可以查看”，但没有完整胚胎遗传学结果。',
    case4: '你已随身带来唐氏筛查和NIPT正式报告。医生询问时必须回答“都带了，可以查看”。',
    case5: '你没有带可供查看的正式外院报告，只记得外院口头建议；可以如实说明没有正式报告。',
    case6: '你已随身带来父亲外院诊断资料和HTT检测报告摘要。医生询问时必须回答“带了，可以查看”。',
    case7: '你已随身带来第一胎患儿出生病历和修复手术记录。医生询问时必须回答“带了，可以查看”。',
    case8: '你已随身带来本人的眼科专科检查资料；母系亲属没有现成遗传检测报告。',
    case9: '你已随身带来既往精液检查报告；尚未完成系统性激素评估和染色体核型分析。'
  }[caseData.caseId] || '请严格按照既往检查信息回答是否携带资料。';
...
13. 对是否带了既往报告、病历或资料的回答必须始终与既往材料携带状态一致，不能随机改口。
...
function isReportRequest(question) {
  ...
  const asksPriorExamHistory = keywordIncludes(q, [
    '以前做过什么检查', '之前做过什么检查', '既往做过什么检查', '以前做过哪些检查', '之前做过哪些检查', '查过什么',
    '舅舅做过检查吗', '舅舅有去做过检查吗', '亲属做过检查吗', '父亲做过检查吗', '孩子做过检查吗',
    '有没有以前的检查', '有没有之前的检查', '既往检查带了吗', '检查资料带了吗', '检查报告带了吗'
  ]);
...
function getCaseSpecificScoringAnchor(caseId) {
  const anchors = {
    case1: `
病例1专项校准：
- 仅知患者舅舅患血友病A时，不能直接断言患者本人有50%携带概率。在简化的X连锁家系推算下，患者母亲约有1/2携带可能，患者本人约有1/4携带可能；实际还要考虑新发变异并优先确认患病亲属的F8变异。
- 若患病亲属资料没有明确家系F8致病变异，不能把患者检查直接表述为“家系致病变异针对性检测”；应先确认患病亲属变异，或说明患者接受的是完整F8分析及其局限。
- 女性携带者并非必然无症状，部分人因凝血因子水平降低或X染色体失活偏倚可有异常出血。
- PGT-M可降低特定家系变异的传递风险，但不能保证胚胎或孩子完全健康；妊娠后仍应提供绒毛取样或羊水穿刺确认的选择。
- 检查路径不要求做满3项。若先复核患病亲属诊断或变异，再用合适的F8检测评估患者，并正确整合结果和生育方案，即使没有另做凝血表型检查也可获得路径满分；凝血因子和VWF检查属于出血风险管理的支持性检查。
`,
    case2: `
病例2专项校准：
- 核心是确认夫妻双方是否为同类地中海贫血携带者。既往血常规报告已经调取并复核时，不应要求再次点击血常规才能得分。
- 满分路径可以是：复核双方红细胞指数，结合HbA2/血红蛋白分析，再用夫妻双方HBB检测确认基因型，最后解释每次妊娠风险和生育选择。
- 铁代谢用于鉴别缺铁，属于按既往资料和临床需要选择的支持项目；不能因为未单独点击铁代谢而机械扣分，也不能要求在3项上限内同时做4项。
`,
    case3: `
病例3专项校准：
- 夫妻双方核型分析是本遗传咨询情境的关键检查。既往流产组织已不可用时，不应因未完成流产组织检测扣分。
- 完成夫妻核型并正确解释平衡易位、妊娠结局及自然妊娠产前诊断/PGT-SR选择，可获得高分或满分。
- 子宫结构及其他非遗传因素评估属于综合反复流产路径，应在建议中体现，但不要求为凑数量全部点击。
`,
    case4: `
病例4专项校准：
- 既往唐筛/NIPT报告已查看时应视为完成报告复核，不要求重复点击。
- 关键路径是说明筛查不是诊断，在知情同意后选择羊水穿刺胎儿染色体诊断并正确解释结果。系统超声是重要补充，但不能替代诊断，也不是路径满分的机械前提。
- 夫妻外周血核型不能替代胎儿诊断，误选应扣分。
`,
    case5: `
病例5专项校准：
- 外周血染色体核型是明确Turner综合征的关键检查。盆腔超声及心血管/肾脏评估用于表型和长期管理。
- 若学生通过核型明确诊断，并主动提出生殖系统及心肾等并发症评估和长期随访，即使未点击全部3项也可获得高分；只做核型却不提出系统管理不应满分。
- 常规代谢检查不能替代核型和结构评估。
`,
    case6: `
病例6专项校准：
- 预测性检测不是必须立即执行的普通检查。先确认父亲HTT结果，再完成检测前遗传咨询、心理状态评估、知情同意及不检测权利，才是优秀路径。
- 不应因为学生没有给无症状来访者立即做本人预测性检测而扣分；相反，未经充分准备直接检测应在路径和伦理维度扣分。
- 头颅影像不能替代家系确认和预测性检测流程。
`,
    case7: `
病例7专项校准：
- 核心是复核第一胎患儿表型，判断综合征性线索，并完成家系/多因素再发风险评估。
- 孕前叶酸与暴露管理、下一胎目标超声属于后续管理方案，不是需要为了满分全部点击的诊断检查。
- 完成患儿资料复核、风险解释和下一胎计划即可获得高分或满分；夫妻核型不应作为孤立性唇腭裂的常规首选。
`,
    case8: `
病例8专项校准：
- 眼科表型评估与mtDNA相关变异检测构成核心诊断路径；母系家族史应优先通过问诊或资料收集获得。
- 若学生已充分追问母系家族史，不应再要求点击“家族史与亲属风险资料收集”才能满分。
- 核型分析不能解释典型LHON线索，误选应扣分。
`,
    case9: `
病例9专项校准：
- 核型分析是明确47,XXY的关键检查；精液分析和性激素用于不育表型、性腺功能与后续管理。
- 既往精液异常已经可靠复核时，不应要求重复检查才能满分。核型确诊后若能结合已有精液资料、提出性激素和生殖医学评估并解释生育选择，可获得高分。
- 配偶检查不能替代男方病因评估，不能因增加检查数量而加分。
`
  };
...
async function generateAIScore(caseData, history = [], completedTests = [], viewedMaterialIds = []) {
  ...
  const scoringContext = {
    ...
    caseSpecificScoringAnchor: getCaseSpecificScoringAnchor(caseData.caseId),
    transcript
  };
...
2. 检查/干预路径25分：问诊后选择与病例相符的关键检查，顺序合理，能够整合结果并形成诊断和咨询建议。不能因为系统显示了结果就视为学生完成了解释。评分依据是关键路径的必要性、顺序、解释和结果整合，不是检查数量；完成1至2项足以解决核心问题时也可以满分。支持性检查不是凑满3项的必做项。做满3项、重复检查或选择干扰项本身不得加分，干扰项应酌情扣分。
...
- 写“学生未说明某要点”前，必须再次检索全部学生发言；如果学生已明确说过，不得列为不足，可评价其完整性或准确性。
- 必须优先执行待评分资料中的caseSpecificScoringAnchor。
